...
fn load_one<T: Any + for<'a> Deserialize<'a>>(
    contents: &str,
    ...
    match item {
        Ok(item) => items.push(toml::from_str::<T>(contents)?),
        Err(err) => println!("{:?} {}", file, err),
    }
    Ok(())
}

fn load<T: Any + for<'a> Deserialize<'a>>(
    asset: &AssetType,
    store: &mut PolyMap<TypeId>,
) -> Result<()> {
    let items: &mut Vec<T> = store.entry(TypeId::of::<T>()).or_insert_with(Vec::new);
...
impl AssetType {
    pub const fn new<T: std::fmt::Debug + Any + for<'a> Deserialize<'a>>(
        kind: &'static str,
    ) -> Self {
        Self {
            kind,
